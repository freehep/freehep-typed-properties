...
public class Assert extends org.junit.Assert {
    ...
    private static boolean compare(float[] reference, float[] data, float epsilon) {
        assert (epsilon >= 0);
        ...
        if (!result) {
            System.err.println("ERROR, l2-norm error " + error + " is greater than epsilon "
                    + epsilon);
        }
        return result;
    }
            ...
            if (isArray(expected) && isArray(actual)) {
                ...
                try {
                    if (expected instanceof Number && actual instanceof Number) {
                        assertEquals(((Number)expected).doubleValue(), ((Number)actual).doubleValue(), delta);                        
                    } else {
    ...
    private static void assertArrayEquals(String message, boolean[] expecteds, boolean[] actuals) {

...
public class ListPropertyConverter<T> implements PropertyConverter<List<T>> {    
    ...
    public void store(Properties p, String key, Class<?> type, List<T> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            if (converter == null) {
                System.err.println("List.store: No Converter defined for '" + entryType
                        + "' of '" + key + "'");
            } else {
    ...
    public void load(TypedProperties properties, String key, Class<?> entryType, String value) {
        ...
        if (m.matches()) {
            ...
            if (converter == null) {
                System.err.println("List.load: No Converter defined for '" + entryType
                        + "' of '" + key + "'");
                ...
                try {
                    ...
                    System.err.println("List.load: Could not load property '" + entryKey
                            + "' with type '" + entryType + "'");
                }
            }
        }

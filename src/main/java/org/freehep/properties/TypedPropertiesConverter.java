 ...
 */
public class TypedPropertiesConverter implements PropertyConverter<TypedProperties> {
     ...
     * @param converters
     */
    public TypedPropertiesConverter(Map<Class<?>, PropertyConverter<?>> converters) {
    ...
    public void load(TypedProperties properties, String key, Class<?> type, String value) {
        ...
        if (m.matches()) {
            ...
            if (!m.group(3).equals("")) {
                ...
                if (converter == null) {
                    System.err.println("TypedProperties.load: No Converter defined for '" + type
                            + "' of '" + key + "'");
                } else {
                    try {
                        converter.load(subProperties, key, type, value);
                    } catch (Exception e) {
                        System.err.println("TypedProperties.load: Could not load property '" + key + "' with type '"
                                + type + "'");
                    }
                }
            }

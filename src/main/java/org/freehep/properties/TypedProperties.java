...
public class TypedProperties {
    ...
    protected <T> void setProperty(String key, Class<?> type, T value) {
        ...
        if (m.matches()) {
            ...
            TypedProperties subProperties = (TypedProperties) properties.get(mainKey);
        ...
        if ((expectedType != null) && !expectedType.isAssignableFrom(type)) {
            throw new ClassCastException("Cannot set property " + key + " with type: " + type
    ...
    private <V> void store(Properties p, String key, Class<?> type, V value) {
        PropertyConverter<V> converter = (PropertyConverter<V>) converters.get(type);
        if (converter == null) {
            System.err.println("Store: No Converter defined for '" + type + "'");
        } else {
    ...
    static {
        ...
        // combined types
        register(TypedProperties.class, new TypedPropertiesConverter(converters));
